...
private:
	 ...
	 */
	std::optional<bool> compare_less_at_x_1vert(std::size_t si1, std::size_t si2, Segment s1, Segment s2) {
		if(s2.s.x == s1.s.x) {
			if(s2.s.y <= s1.s.y) {
				return false;
			}
	 ...
	 */
	std::optional<bool> compare_less_at_x_novert(std::size_t si1, std::size_t si2, Segment s1, Segment s2) {
		if(s2.s.x == s1.s.x) {
			if(s1.s.y > s2.s.y) {
				return false;
			}
